...
describe('setupDocs', () => {
  ...
  let mockApp: Partial<NestExpressApplication>;
  let mockHttpAdapter: { get: Mock; getInstance: Mock };
  ...
  beforeEach(() => {
    ...
    mockApp = {
      getHttpAdapter: vi.fn().mockReturnValue(mockHttpAdapter)
    };
  });
  ...
  it('should set all provided configuration options', () => {
    setupDocs(mockApp as NestExpressApplication, {
      config: {
        contact: {
  ...
  it('should register the document at the specified path', () => {
    SwaggerModule.createDocument.mockReturnValueOnce('CREATED_DOCUMENT');
    setupDocs(mockApp as NestExpressApplication, {
      config: {
        title: 'Test API'
      },
    ...
    const [path, callback] = mockHttpAdapter.get.mock.lastCall ?? [];
    expect(path).toBe('/api-docs.json');
    expect(callback).toBeTypeOf('function');
    const mockRes = { send: vi.fn() };
    callback(null, mockRes);
    expect(mockRes.send).toHaveBeenCalledWith('CREATED_DOCUMENT');
  });
});
